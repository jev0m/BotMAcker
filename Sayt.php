...
'chat_id'=>$chat_id, 
'text' => "
- اهلا بك في بوت الهمسه السريه
- يمكنك ارسال رساله سريه لصديقك في المجموعات او القنوات ولا يمكن لاحد الاطلاع عليها غيركم
- لمعرفة كيفية استخدام البوت ارسل /help
",
]);
} 
elseif($text == '/start' and $chat_id != $admin and (in_array($from_id,$ebu))==false){
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text' =>"
- اهلا بك عزيزي في بوت السايت
- يمكنك ارسال رسالتك بصوره سريه الى الادمن
- يمكنك ارسال - نص، فيديو، ملف، بصمه، صوت، صوره، الخ... 
",
'parse_mode'=>"MarkDown",
...
if($text && $text !== "/start" and $chat_id != $admin && (in_array($from_id,$ebu))==false){
 bot('sendMessage',[
'chat_id'=>$sudo,
'text'=>"- الرساله ؛ $text",
]);
bot('sendMessage',[
 'chat_id'=>$chat_id,
 'text'=>"- تم ارسال رسالتك الى المطور بدون معرفة هويتك", 
]);
} 
elseif($message->photo and $chat_id != $admin && (in_array($from_id,$ebu))==false){
...
if($message->video and $chat_id != $admin && (in_array($from_id,$ebu))==false){
...
if($message->document and $chat_id != $admin && (in_array($from_id,$ebu))==false){

 bot('senddocument',[
'chat_id'=>$sudo,'document'=>$message->document->file_id,'caption'=>$cap,
]);
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"- تم ارسال رسالتك الى المطور بدون معرفة هويتك", 
...
if($message->voice and $chat_id != $admin && (in_array($from_id,$ebu))==false){
 bot('sendvoice',[
  'chat_id'=>$sudo,
...
if($message->audio and $chat_id != $admin && (in_array($from_id,$ebu))==false){
 bot('sendaudio',[
  'chat_id'=>$sudo,
...
if($message->sticker and $chat_id != $admin && (in_array($from_id,$ebu))==false){
 bot('sendsticker',[
'chat_id'=>$sudo,'sticker'=>$message->sticker->file_id,
]);
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"- تم ارسال رسالتك الى المطور بدون معرفة هويتك", 
